...
canvas.addEventListener(
  ...
  function (evt) {
    ...
    for (var i = 0; i < 64; i++) {
      ...
      if (
        xVal >= myItems[i].x &&
        xVal <= myItems[i].xfull &&
        yVal >= myItems[i].y &&
        yVal <= myItems[i].yfull
      ) {
        
        n = (i + 1).toString();
        //ctx.drawImage(img1,myItems[i].x,myItems[i].y,withchair,heightchair);
      }
    }
    if (n != null) {
      $.post('/seatingmap/update', { id: n }, function () {
        console.log('success');
      });
      setTimeout(function () {
        window.location.replace('/seatingmap');
      }, 500);
    }
  },
  false,
